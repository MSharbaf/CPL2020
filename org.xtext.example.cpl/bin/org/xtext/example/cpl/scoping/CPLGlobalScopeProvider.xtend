...
class CPLGlobalScopeProvider extends ImportUriGlobalScopeProvider {
	...
	override protected getImportedUris(Resource resource) {
		return cache.get(CPLGlobalScopeProvider.getSimpleName(), resource, new Provider<LinkedHashSet<URI>>() {
			override get() {
...
//				val models = resourceDescription.getExportedObjectsByType(MyDslPackage.Literals.ROOT_MODEL)
				val models = resourceDescription.getExportedObjectsByType(CPLPackage.Literals.TOP_LEVEL_CP)
				
				models.forEach[
					val userData = getUserData(CPLResourceDescriptionStrategy.INCLUDES)
					if(userData !== null) {
